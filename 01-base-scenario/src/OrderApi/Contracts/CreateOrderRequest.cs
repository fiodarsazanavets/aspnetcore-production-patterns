...
public sealed record OrderResponse(Guid OrderId, string CustomerId, string Status, DateTimeOffset CreatedAt, IReadOnlyList<OrderLineResponse> Lines, ShippingQuoteResponse? ShippingQuote);
public sealed record OrderLineResponse(string Sku, int Quantity);
public sealed record ShippingQuoteResponse(string Carrier, decimal Amount, DateTimeOffset ExpiresAt);
